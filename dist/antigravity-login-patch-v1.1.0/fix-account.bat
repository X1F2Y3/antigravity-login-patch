...
rem CLI 保存到当前用户目录(%LOCALAPPDATA%), 不依赖专属路径; 可用 AG_CLI_DIR 覆盖
set "CLI_DIR=%LOCALAPPDATA%\agy-cli"
if not "%AG_CLI_DIR%"=="" set "CLI_DIR=%AG_CLI_DIR%"
set "CLI_EXE=%CLI_DIR%\antigravity.exe"

echo  [配置] 代理 http://%PROXY_IP%:%PROXY_PORT%   CLI: %CLI_DIR%
echo  [覆盖] 换端口: set AG_PROXY_PORT=你的端口  或  直接传参: fix-account.bat 你的端口
echo.
